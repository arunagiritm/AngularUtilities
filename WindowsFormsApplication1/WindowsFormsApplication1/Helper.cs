    ...
    public static class Helper
    {
       ...
       public  enum JSONTYPE
         ...
         public static void JsonReader(string json, ref DataTable dt)
        {
         ...
         private static void BuildTree(Dictionary<string, object> dictionary, ref DataTable dt,Int32 rkey,Int32 pid)
        {
                ...
                try
                {
                        ...
                        item.Value.GetType().Name == "ArrayList") //parent node
                    {
                        ...
                        if (item.Value.GetType().Name == "Dictionary`2")
                        {
                            ...
                            //jstack.Push(pid);
                            BuildTree(dictionary, ref dt, rkey, pid);
                            //jstack.Pop();
                            //pid = jstack.Peek();
                        }
                        else if (item.Value.GetType().Name == "ArrayList")
                        {
                            ...
                            for (int i = 0; i < arrItem.Count; i++)
                            {
                                ...
                                {
                                    foreach (KeyValuePair<string, object> subitem in (Dictionary<string, object>)arrItem[i])
                                    {
                                        dr = dt.NewRow();
                                        dr["Id"] = rkey;
                                        dr["Node"] = subitem.Key;
